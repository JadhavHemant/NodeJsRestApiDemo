...
// Create a model for personal information
const PersonalInfo = mongoose.model('PersonalInfo', personalInfoSchema);

// Define schema for managing next user ID
...
// Create a model for ID counter
const IDCounter = mongoose.model('IDCounter', idCounterSchema);

// Initialize the ID counter if it does not exist
async function initializeIdCounter() {
  const counter = await IDCounter.findOne();
  if (!counter) {
      await IDCounter.create({ nextId: 1 });  // Start the counter with 1 if not initialized
      console.log('IDCounter initialized with nextId: 1');
  } else {
      console.log('IDCounter already initialized');
  }
}

// Get the next auto-incremented ID
async function getNextId() {
  let idCounter = await IDCounter.findOne();
  if (!idCounter) {
      // Ensure that the IDCounter is created if it somehow doesn't exist
      idCounter = await IDCounter.create({ nextId: 1 });
  }
  
  idCounter = await IDCounter.findOneAndUpdate(
      {},
      { $inc: { nextId: 1 } },  // Increment nextId by 1
      { new: true }
  );
...
app.post('/api/personalInfo', async (req, res) => {
  try {
      ...
      // Create the new personal information entry with the generated ID
      const personalInfoData = { ...req.body, id: newId };
...
app.get('/api/personalInfo/:id', async (req, res) => {
  try {
      const personalInfo = await PersonalInfo.findOne({ id: req.params.id });
      if (!personalInfo) {
...
// Update personal information by ID (PUT)
...
// Partially update personal information by ID (PATCH)
app.patch('/api/personalInfo/:id', async (req, res) => {
  try {
      const personalInfo = await PersonalInfo.findOneAndUpdate(
          { id: req.params.id },
          req.body,
...
app.delete('/api/personalInfo/:id', async (req, res) => {
  try {
      const personalInfo = await PersonalInfo.findOneAndDelete({ id: req.params.id });
      if (!personalInfo) {
          return res.status(404).json({ message: 'Personal information not found' });
      }
